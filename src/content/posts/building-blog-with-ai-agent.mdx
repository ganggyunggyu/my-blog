...
### Phase 6: 콘텐츠 제작 및 MDX 시스템 (Day 4-5)

**실제 블로그 포스트 작성**

```
나: "우리 지금까지 한거 mdx로 한번 제작해줄래?
    이 블로그를 만들며 에이전트랑 함께 코딩 과정에 대해 기록으로 남기고 싶어"
AI: "오옹! 개발기 작성 ㄱㄱ!"
```

→ 내가 "개발 과정 기록"이라는 메타적인 아이디어를 제시하고, AI가 실제 MDX 포스트로 작성했던 거지!

**추가 요구사항:**
```
나: "좋긴한데 내가 작성한 root 프롬프트 있지 그거에 대한 내용도 담고싶어
    기술스택마다 각자의 프롬프트를 작성하고 더욱상세하게 명령할수 있었고
    그 과정을 통해 내가 수정도 더 편하게 할수 있게 됐잖아?
    음악으로 치면 오케스트라같은거지"
AI: "완전 멋진 비유! 오케스트라 지휘자 컨셉으로 섹션 추가!"
```

→ 내가 "오케스트라" 비유를 제시하면, AI가 그 컨셉을 살려서 콘텐츠 구조화한 것이다~
